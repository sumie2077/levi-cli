...
BASE_URL = "https://cdn.levi.com/binaries/levi-cli"
...
async def _do_update(*, print: bool, check_only: bool) -> UpdateResult:
    ...
    async with new_client_session() as session:
        ...
        filename = f"levi-{latest_version}-{target}.tar.gz"
        download_url = f"{BASE_URL}/{latest_version}/{filename}"

        with tempfile.TemporaryDirectory(prefix="levi-cli-") as tmpdir:
            ...
            try:
                ...
                for root, _, files in os.walk(tmpdir):
                    if "levi" in files:
                        binary_path = os.path.join(root, "levi")
                        break
                if not binary_path:
                    logger.error("Binary 'levi' not found in archive.")
                    _print("[red]Binary 'levi' not found in archive.[/red]")
                    return UpdateResult.FAILED
            except Exception:
    ...
    _print("[yellow]Restart Levi CLI to use the new version.[/yellow]")
