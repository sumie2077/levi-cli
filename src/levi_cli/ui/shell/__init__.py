...
from levi_cli.soul import LLMNotSet, LLMNotSupported, MaxStepsReached, RunCancelled, Soul, run_soul
from levi_cli.soul.levisoul import LeviSoul
from levi_cli.ui.shell.console import console
from levi_cli.ui.shell.metacmd import MetaCommand, get_meta_command
from levi_cli.ui.shell.prompt import CustomPromptSession, PromptMode, toast
from levi_cli.ui.shell.replay import replay_recent_history
from levi_cli.ui.shell.update import LATEST_VERSION_FILE, UpdateResult, do_update, semver_tuple
from levi_cli.ui.shell.visualize import visualize
from levi_cli.utils.signals import install_sigint_handler
from levi_cli.utils.term import ensure_new_line
from levi_cli.wire.message import StatusUpdate
...
class Shell:
    ...
    async def run(self, command: str | None = None) -> bool:
        ...
        if isinstance(self.soul, LeviSoul):
            await replay_recent_history(self.soul.context.history)

        with CustomPromptSession(
            status_provider=lambda: self.soul.status,
            model_capabilities=self.soul.model_capabilities or set(),
            initial_thinking=isinstance(self.soul, LeviSoul) and self.soul.thinking,
        ) as prompt_session:
            while True:
                try:
                    ensure_new_line()
    ...
    async def _run_meta_command(self, command: MetaCommand, args: list[str]) -> None:
        ...
        if command.levi_soul_only and not isinstance(self.soul, LeviSoul):
    ...
    async def _run_soul_command(
        ...
        try:
            if isinstance(self.soul, LeviSoul) and thinking is not None:
            ...
            await run_soul(
                ...
                cancel_event,
                self.soul.wire_file if isinstance(self.soul, LeviSoul) else None,
            )
            return True
        except LLMNotSet:
    ...
    #         while True:
    #             toast(
    #                 "new version found, run `uv tool upgrade levi-cli` to upgrade",
    #                 topic="update",
    #                 duration=30.0,
    #             )
...
def _print_welcome_info(name: str, info_items: list[WelcomeInfoItem]) -> None:
    ...
    #     from levi_cli.constant import VERSION as current_version
    ...
    #                 "Please run `uv tool upgrade levi-cli` to upgrade.[/yellow]"
    #             )
    #         )
