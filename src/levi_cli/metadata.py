...
def get_metadata_file() -> Path:
    return get_share_dir() / "levi.json"
...
class WorkDirMeta(BaseModel):
    ...
    def sessions_dir(self) -> Path:
        ...
        session_dir = get_share_dir() / "sessions" / path_md5
        session_dir.mkdir(parents=True, exist_ok=True)
        return session_dir


class Metadata(BaseModel):
    """Levi metadata structure."""
    ...
    def get_work_dir_meta(self, path: Path) -> WorkDirMeta | None:  # 改这里
