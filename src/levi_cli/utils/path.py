...
async def list_directory(work_dir: Path) -> str:  
    """Return an ``ls``-like listing of *work_dir*.

    This helper is used mainly to provide context to the LLM (for example
    ``LEVI_WORK_DIR_LS``) and to show top-level directory contents in tools.
...
def is_within_directory(path: Path, directory: Path) -> bool:  
    ...
    Both arguments should already be canonicalized (e.g. via path.resolve()).
    """
