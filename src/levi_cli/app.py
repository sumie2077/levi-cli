...
from levi_cli.cli import InputFormat, OutputFormat
from levi_cli.config import LLMModel, LLMProvider, load_config
from levi_cli.llm import augment_provider_with_env_vars, create_llm
from levi_cli.session import Session
from levi_cli.share import get_share_dir
from levi_cli.soul import LLMNotSet, LLMNotSupported, run_soul
from levi_cli.soul.agent import Runtime, load_agent
from levi_cli.soul.context import Context
from levi_cli.soul.levisoul import LeviSoul
from levi_cli.utils.logging import StreamToLogger, logger
from levi_cli.utils.path import shorten_home
from levi_cli.wire import Wire, WireUISide
from levi_cli.wire.message import WireMessage


def enable_logging(debug: bool = False) -> None:
    logger.remove()  # Remove default stderr handler
    logger.enable("levi_cli")
    if debug:
        logger.enable("kosong")
    logger.add(
        get_share_dir() / "logs" / "levi.log",
...
class LeviCLI:
    ...
    async def create(
        ...
        *,
        ...
        Raises:
            ...
            ConfigError(LeviCLIException): When the configuration is invalid.
            AgentSpecError(LeviCLIException): When the agent specification is invalid.
        """
        ...
        if not model:
            ...
            provider = LLMProvider(type="local", base_url="", api_key=SecretStr(""))
        #默认llmprovider为local
        ...
        soul = LeviSoul(agent, context=context)
        try:
        ...
        return LeviCLI(soul, runtime, env_overrides)
    ...
    @property
    def soul(self) -> LeviSoul:
        """Get the LeviSoul instance."""
        return self._soul
    ...
    async def _env(self) -> AsyncGenerator[None]:
        ...
        await asyncio.to_thread(os.chdir, self._runtime.session.work_dir)
        try:
            ...
            await asyncio.to_thread(os.chdir, original_cwd)
    ...
    async def run(
        ...
        """
        Run the Levi CLI instance without any UI and yield Wire messages directly.
        ...
        Yields:
            WireMessage: The Wire messages from the `LeviSoul`.
    ...
    async def run_shell(self, command: str | None = None) -> bool:
        """Run the Levi CLI instance with shell UI."""
        from levi_cli.ui.shell import Shell, WelcomeInfoItem
        ...
        if base_url := self._env_overrides.get("LEVI_BASE_URL"):
            welcome_info.append(
                WelcomeInfoItem(
                    name="API URL",
                    value=f"{base_url} (from LEVI_BASE_URL)",
                    level=WelcomeInfoItem.Level.WARN,
                )
            )
        if self._env_overrides.get("LEVI_API_KEY"):
            welcome_info.append(
                WelcomeInfoItem(
                    name="API Key",
                    value="****** (from LEVI_API_KEY)",
                    level=WelcomeInfoItem.Level.WARN,
                )
            )
        ...
        elif "LEVI_MODEL_NAME" in self._env_overrides:
            welcome_info.append(
                WelcomeInfoItem(
                    name="Model",
                    value=f"{self._soul.model_name} (from LEVI_MODEL_NAME)",
                    level=WelcomeInfoItem.Level.WARN,
                )
            )
        else:
    ...
    async def run_print(
        ...
        """Run the Levi CLI instance with print UI."""
        from levi_cli.ui.print import Print
    ...
    async def run_acp(self) -> None:
        """Run the Levi CLI instance as ACP server."""
        from levi_cli.ui.acp import ACP
    ...
    async def run_wire_stdio(self) -> None:
        """Run the Levi CLI instance as Wire server over stdio."""
        from levi_cli.ui.wire import WireOverStdio
