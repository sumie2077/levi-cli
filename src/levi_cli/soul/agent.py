...
from levi_cli.agentspec import load_agent_spec
from levi_cli.config import Config
from levi_cli.llm import LLM
from levi_cli.session import Session
from levi_cli.soul.approval import Approval
from levi_cli.soul.timemachine import TimeMachine
from levi_cli.soul.toolset import LeviToolset, ToolType
from levi_cli.tools import SkipThisTool
from levi_cli.utils.environment import Environment
from loguru import logger
from levi_cli.utils.path import list_directory
...
class BuiltinSystemPromptArgs:
    ...
    LEVI_WORK_DIR_LS: str
    """The directory listing of current working directory."""
    LEVI_AGENTS_MD: str  # TODO: move to first message from system prompt
    """The content of AGENTS.md."""


async def load_agents_md(work_dir: Path) -> str | None:
    paths = [
        work_dir / "AGENTS.md",
        work_dir / "agents.md",
    ]
...
class Runtime:
    ...
    async def create(
        ...
        return Runtime(
            ...
            session=session,
            builtin_args=BuiltinSystemPromptArgs(
                LEVI_NOW=datetime.now().astimezone().isoformat(),
                LEVI_WORK_DIR=session.work_dir,
                LEVI_WORK_DIR_LS=ls_output,
                LEVI_AGENTS_MD=agents_md or "",
            ),
            time_machine=TimeMachine(),
    ...
    def copy_for_fixed_subagent(self) -> Runtime:
        ...
        return Runtime(
            ...
            time_machine=TimeMachine(),  # subagent must have its own TimeMachine
...
async def load_agent(
    ...
    *,
    ...
    tool_deps = {
        ...
        TimeMachine: runtime.time_machine,
...
# TODO: maybe move to `LeviToolset`
def _load_tools(
    toolset: LeviToolset,
    tool_paths: list[str],
...
async def _load_mcp_tools(
    ...
    from levi_cli.tools.mcp import MCPTool
